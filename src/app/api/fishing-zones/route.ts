...
export async function GET(request: NextRequest) {
  ...
  const isPanIndia = region === 'all_india' || params.get('all_india') === 'true';

  if (isPanIndia) {
    const zones = generatePanIndiaFishingZones();
    return NextResponse.json({
      location: { lat: 18.0, lon: 80.0, name: 'Pan-India All Coasts' },
      conditions: {
        sst: 28.2,
        chlorophyll: 2.8,
        currentSpeed: 1.0,
        dataStatus: 'LIVE',
        source: 'ISRO MOSDAC Satellite Network (11 Sectors)',
      },
      zones,
