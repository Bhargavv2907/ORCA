  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-2.5 py-2 space-y-2.5">
          ...
          <div className="pb-1">
            <button
              ...
              className={cn(
                'w-full flex items-center gap-2 px-2.5 py-2 rounded-xl text-xs font-black transition-all bg-gradient-to-r from-red-600 via-rose-600 to-red-700 hover:from-red-500 hover:to-rose-500 text-white shadow-md shadow-red-600/25 border border-red-400/40 glow-danger animate-pulse cursor-pointer',
                collapsed && 'justify-center px-0'
              )}
            >
              <AlertOctagon className="w-4 h-4 shrink-0 text-white" />
              {!collapsed && <span className="truncate tracking-wider uppercase">{t('EMERGENCY SOS', language)}</span>}
            </button>
          </div>
        ...
        {/* Pinned Bottom Controls (Collapse) */}
        <div className="p-2.5 border-t border-navy-700/30 bg-navy-950/40 shrink-0 space-y-1">
          <button
            ...
            title={collapsed ? t('Expand', language) || 'Expand' : undefined}
          >
            {collapsed ? <ChevronRight className="w-3.5 h-3.5" /> : <ChevronLeft className="w-3.5 h-3.5" />}
            {!collapsed && <span>{t('Collapse', language)}</span>}
          </button>
        </div>
      </aside>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 glass border-t border-navy-700/30 px-2 py-1 flex items-center justify-around">
        {[NAV_ITEMS[0], NAV_ITEMS[1], NAV_ITEMS[2], NAV_ITEMS[3], NAV_ITEMS[8]].filter(Boolean).map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={cn(
          ...
          >
            ...
            <span>{t(item.label.split(' ')[0], language)}</span>
          </Link>
        ))}
      </nav>
